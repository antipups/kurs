class mt:
    ...
    def first_condition(self):
        """
            ...
            Порядок именно такой, вначале если нужно меняем букву, после меняем направление и уже после меняем состояние.
        """
        pass
    ...
    def heart(self, word, cursor):
        ...
        while True:     # бесконечный цикл, это и есть некая головка, которая будет ходить по литерам
            ...
            if self.direction == '>':
                ...
                return ''.join(word[1:word.index('L', 1)])      # выходим из машины, соединяем полученное на выходе слово в строку
                                                # и возвращаем его
